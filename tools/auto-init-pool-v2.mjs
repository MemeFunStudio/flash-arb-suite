...
// required derived accounts
const [POOL, POOL_BUMP]=PublicKey.findProgramAddressSync([Buffer.from('pool'), MINT.toBuffer(), OWNER.toBuffer()], PROGRAM);

// candidate vault authority PDAs
const labels=['vault','vault_authority','authority','auth','settle','settlement','va'];
const parts=[
  ['POOL'],['POOL','MINT'],['MINT','OWNER'],['GLOBAL'],['GLOBAL','MINT'],['GLOBAL','POOL'],['MINT'],['OWNER']
];
function partToBuf(name){ if(name==='POOL')return POOL.toBuffer(); if(name==='MINT')return MINT.toBuffer(); if(name==='OWNER')return OWNER.toBuffer(); if(name==='GLOBAL')return GLOBAL.toBuffer(); throw new Error('bad part'); }
const candidates=[];
for(const L of labels){ for(const P of parts){ const seeds=[Buffer.from(L)]; for(const seg of P) seeds.push(partToBuf(seg)); const va=PublicKey.findProgramAddressSync(seeds,PROGRAM)[0].toBase58(); candidates.push([L+'+'+P.join('+'),va]); } }
const uniq=new Map(); for(const [k,v] of candidates){ if(!uniq.has(v)) uniq.set(v,k); }
const ordered=[...uniq.entries()].map(([addr,label])=>[label,addr]);

function mapArgVal(a){ const key=UP(a.name); let val=env[key]; if(val===undefined && /profit/i.test(a.name)) val=env.MIN_PROFIT_BPS; if(val===undefined && /pool.*bump/i.test(a.name)) val=String(POOL_BUMP); if(val===undefined && /pool.*id/i.test(a.name)) val=env.POOL_ID; if(val===undefined && /mint/i.test(a.name)) val=env.MINT; if(val===undefined) throw new Error('MISSING_ARG:'+a.name); return enc(a.type,val); }
function keysFor(vaultAuth, vault){ const mapName=(n)=>{ const P=UP(n); if(P==='OWNER'||/owner|authority/i.test(n)) return OWNER; if(P==='GLOBAL'||/global/i.test(n)) return GLOBAL; if(P==='POOL'||/^pool$/i.test(n)) return POOL; if(/vault.*authority/i.test(n)) return vaultAuth; if(/^vault$/i.test(n)) return vault; if(/mint/i.test(n)) return MINT; if(/token.?program/i.test(n)) return TOKEN_PROGRAM; if(/system.?program/i.test(n)) return SystemProgram.programId; if(/rent/i.test(n)) return new PublicKey(env.SYSVAR_RENT||'SysvarRent111111111111111111111111111111111'); throw new Error('MISSING_ACCOUNT:'+n); }; return (ixMeta.accounts||[]).map(ac=>{ const pk = mapName(ac.name); const nameLower = String(ac.name).toLowerCase(); const isSigner = Boolean(ac.isSigner) || /owner|authority/.test(nameLower); const isWritable = Boolean(ac.isMut) || /pool|global|vault|vault_authority/.test(nameLower); return {pubkey:pk,isSigner,isWritable}; }); }

async function runOne(label, vaPk){
  ...
  if(!ai){
    const tx1=new Transaction().add(createAssociatedTokenAccountInstruction(payer.publicKey, ata, vaPk, MINT, TOKEN_PROGRAM, ASSOCIATED_TOKEN_PROGRAM_ID));
    ...
    console.log('ATA_CREATE_SIG='+sig1); console.log('ATA_EXPLORER=https://explorer.solana.com/tx/'+sig1+'?cluster=devnet');
  }
  const argBufs=[]; for(const a of (ixMeta.args||[])) argBufs.push(mapArgVal(a));
  ...
  console.log('TRY_LABEL='+label); console.log('VAULT_AUTHORITY='+vaPk.toBase58()); console.log('VAULT='+ata.toBase58()); console.log('POOL_CREATE_SIG='+sig); console.log('EXPLORER=https://explorer.solana.com/tx/'+sig+'?cluster=devnet');
  console.log('POOL_OWNER='+(poolInfo?poolInfo.owner.toBase58():'')); console.log('POOL_DATALEN='+(poolInfo?poolInfo.data.length:0)); console.log('POOL_OK='+(ok?'YES':'NO'));
  return ok;
}

(async()=>{
  for(const [label,addr] of ordered){
    try{ const ok = await runOne(label, new PublicKey(addr)); if(ok){ console.log('WIN_LABEL='+label); process.exit(0); } }catch(e){ console.log('TRY_FAIL='+label); console.log('ERR='+String(e).slice(0,240)); }
  }
