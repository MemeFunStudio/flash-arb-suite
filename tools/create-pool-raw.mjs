...
const MIN_BPS=parseInt(process.env.MIN_PROFIT_BPS||"5",10);
const data=Buffer.concat([disc("create_pool"),Buffer.alloc(2,0)]);
data.writeUInt16LE(MIN_BPS,8);
const keys=[
...
vtx.sign([payer]);
const sig=await sendAndConfirmTransaction(conn,vtx,{skipPreflight:false,commitment:"confirmed"});
console.log("SIG="+sig);
